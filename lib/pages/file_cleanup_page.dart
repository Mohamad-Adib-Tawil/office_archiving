...
class _FileCleanupPageState extends State<FileCleanupPage> with TickerProviderStateMixin {
  ...
  Future<void> _scanForIssues() async {
    ...
    if (mounted) {
      final totalIssues = _duplicateFiles.length + _brokenFiles.length + _largeFiles.length;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            totalIssues > 0 
  ...
  Future<void> _cleanupBrokenFiles() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('تم تنظيف $cleanedCount ملف مكسور'),
        ...
        children: [
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              label: Text(_isScanning ? AppLocalizations.of(context).scanning : AppLocalizations.of(context).start_scan),
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(
            width: double.infinity,
            child: ElevatedButton.icon(
              onPressed: (_brokenFiles.isNotEmpty && !_isCleaningUp) ? _cleanupBrokenFiles : null,
              icon: _isCleaningUp 
                ? const SizedBox(
                    width: 20,
                    height: 20,
              ...
              label: Text(_isCleaningUp ? AppLocalizations.of(context).cleaning : AppLocalizations.of(context).auto_cleanup),
              style: ElevatedButton.styleFrom(
                backgroundColor: _brokenFiles.isNotEmpty ? Colors.green : Colors.grey,
                foregroundColor: Colors.white,
                padding: const EdgeInsets.symmetric(vertical: 16),
                shape: RoundedRectangleBorder(
                  borderRadius: BorderRadius.circular(12),
